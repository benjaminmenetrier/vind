...
#include "vind/LinearModel/LinearModelNumPy.h"
...
static LinearModelMaker<LinearModelNumPy> makerNumPy_("NumPy");

// -----------------------------------------------------------------------------

LinearModelNumPy::LinearModelNumPy(const Geometry & geom,
                                   const eckit::Configuration & config)
  : timeResolution_(config.getString("time step")),
  comm_(geom.getComm()),
  pythonModule_(config.getString("python module")) {
  oops::Log::trace() << classname() << "::LinearModelNumPy starting" << std::endl;
  ...
  oops::Log::trace() << classname() << "::LinearModelNumPy done" << std::endl;
}

// -----------------------------------------------------------------------------

LinearModelNumPy::~LinearModelNumPy() {
  oops::Log::trace() << classname() << "::~LinearModelNumPy starting" << std::endl;
...
void LinearModelNumPy::setTrajectory(const State & xx,
                                     State & xlr,
...
void LinearModelNumPy::initializeTL(Increment & dx) const {
...
void LinearModelNumPy::stepTL(Increment & dx,
  ...
  if (comm_.rank() == 0) {
    ...
    for (const auto & var : dx.variables()) {
      auto field = globalTraj[var.name()];
      if (pythonModule_ == "DDL95") {
        // Create numpy array
    ...
    for (const auto & var : dx.variables()) {
      auto field = globalIncr[var.name()];
      if (pythonModule_ == "DDL95") {
        // Create numpy array
        const int nx = (*initData_)["nx"].cast<size_t>();
        const int ny = (*initData_)["ny"].cast<size_t>();
    ...
    for (const auto & var : dx.variables()) {
      ...
      if (pythonModule_ == "DDL95") {
        ...
        const auto dataView = stepData["increment"][var.name().c_str()]
...
void LinearModelNumPy::finalizeTL(Increment & dx) const {
...
void LinearModelNumPy::initializeAD(Increment & dx) const {
...
void LinearModelNumPy::stepAD(Increment & dx,
                              const ModelAuxIncrement & dxAux) const {
  ...
  // Gather data on main processor
  fs.gather(traj_.at(dx.validTime()-timeResolution_).fieldSet(), globalTraj);
  fs.gather(dx.fieldSet(), globalIncr);
...
void LinearModelNumPy::finalizeAD(Increment & dx) const {
...
void LinearModelNumPy::print(std::ostream & os) const {
  oops::Log::trace() << classname() << "::print starting" << std::endl;

  os << "NumPy linear model:" << std::endl;
